...
- `migrate-plan` reports `mode=declared-capability` or `mode=v2-manifest`.
...
Full cutover requires a capability-backed plan mode. Legacy projects that still
use `.harness-capabilities.json` should report `mode=declared-capability`.
Projects that already use the v2 manifest structure may instead report
`mode=v2-manifest`. Either mode is only a candidate for full cutover; the final
session must still pass `migrate-verify --full-cutover`.
...
- `mode=declared-capability` or `mode=v2-manifest`
- `recommendedCapabilities=[]`
