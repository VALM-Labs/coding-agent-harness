...
- `migrate-plan` 报告 `mode=declared-capability` 或 `mode=v2-manifest`。
...
Full cutover 需要 capability-backed plan mode。仍使用
`.harness-capabilities.json` 的 legacy 项目应报告 `mode=declared-capability`；
已经使用 v2 manifest 结构的项目可以报告 `mode=v2-manifest`。这两种 mode
都只是 full cutover 候选状态；最终 session 仍必须通过
`migrate-verify --full-cutover`。
...
- `mode=declared-capability` 或 `mode=v2-manifest`
- `recommendedCapabilities=[]`
